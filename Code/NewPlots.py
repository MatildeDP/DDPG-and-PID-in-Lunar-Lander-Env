...
def ConfInt(scores, conf):
    ...
    sns.scatterplot(x=np.arange(1, 13, 1), y=scores,
                    hue=["Iter. 1", "Iter. 2", "Iter. 3", "Iter. 4", "Iter. 5", "Iter. 6", "Iter. 7", "Iter. 8", "Iter. 9", "Iter. 10", "Iter. 11", "Iter. 12"],
                    palette=c, legend=False)
    plt.errorbar(x=np.arange(1, 13, 1), y=scores, yerr=conf, fmt='none', color=c)
    plt.xlim(0, 13)
